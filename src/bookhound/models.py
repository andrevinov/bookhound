...
class PersistedLicenseEvidence(BookhoundModel):
    id: int | None = Field(default=None, gt=0)
    evidence: LicenseEvidence


class PersistedDownloadCandidate(BookhoundModel):
    candidate: RawCandidate
    canonical_url: str
    document_id: int = Field(gt=0)
    document_url_id: int = Field(gt=0)
    license_evidence: list[PersistedLicenseEvidence] = Field(default_factory=list)
...
class LicenseDecision(BookhoundModel):
    ...
    unknown_license_confirmed: bool = Field(default=False, exclude=True)
