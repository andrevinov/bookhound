...
def test_safe_pdf_detection_returns_false_for_invalid_urls() -> None:
    assert safe_is_direct_pdf_url("not a url") is False


def test_canonicalize_url_or_raw_keeps_invalid_urls_unchanged() -> None:
    assert canonicalize_url_or_raw("not a url") == "not a url"


def test_url_domain_returns_lowercase_hostname_or_empty_string() -> None:
    assert url_domain("https://Example.ORG/reports/paper.pdf") == "example.org"
    assert url_domain("/relative/path.pdf") == ""


def test_title_from_url_uses_last_path_segment_or_raw_url() -> None:
    assert title_from_url("https://example.org/reports/paper.pdf") == "paper.pdf"
    assert title_from_url("https://example.org") == "https://example.org"


def test_supported_url_detection_reuses_bookhound_url_rules() -> None:
    assert is_supported_url("https://example.org/paper.pdf") is True
    assert is_supported_url("ftp://example.org/paper.pdf") is False
